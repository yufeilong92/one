...
import com.zzzh.akhalteke.ui.verify.CarLengthActivity
import com.zzzh.akhalteke.ui.verify.CarNumberActivity
import com.zzzh.akhalteke.ui.verify.CarTypeActivity
...
import kotlinx.android.synthetic.main.activity_car_new.*
 ...
 * @Purpose :车辆认证
 */
class CarNewActivity : SelectorImageActivity(), CarContract.View{
    ...
    fun initView() {
        mPaths = hashMapOf()
        iv_car_new_id_z.setOnClickListener {
            //行驶证
            toShowDialog(DataMessageVo.CarIdZ)
        }
        iv_car_new_id_f.setOnClickListener {
            //经营证件照
            toShowDialog(DataMessageVo.CarIdF)
        }
        tv_car_new_color.setOnClickListener {
            //车牌颜色
            ...
            val selectCarColorDialog =
                    SelectCarColorDialog(this, object : SelectCarColorDialog.SelectCarColorInterface {
                        override fun onItemClickListener(id: Int) {
                            tv_car_new_color.text = cars[id - 1]
                            //todo处理用户选中牌照
        ...
        tv_car_new_number.setOnClickListener {
            jumpToFoResulBU(
                    CarNumberActivity::class.java,
                    CARLENGTHNUMBER
            )
        }
        tv_car_new_type.setOnClickListener {
            //车型
            jumpToFoResulBU(
        ...
        tv_car_new_leng.setOnClickListener {
            //车长
            jumpToFoResulBU(
    ...
    fun submit() {
        ...
        val carnumber = Utils.getObjToStr(tv_car_new_number)
        ...
        val length = Utils.getObjToStr(tv_car_new_leng)
        if (StringUtil.isEmpty(length)) {
            showToast(getString(R.string.please_car_length))
            return
        }
        val kg = Utils.getObjToStr(et_car_new_kg)
...
//            ll_driver_jiashi.visibility= View.GONE
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == CARTYPENUMBER) {//车型
            ...
            tv_car_new_number.text = replace
            ...
            if (!StringUtil.isEmpty(carLengtName)) {
                tv_car_new_leng.text = carLengtName + "米"
            }
        }
    }
...
//        ll_driver_jiashi.visibility= View.VISIBLE
        tv_car_new_number.text=data.plateNumber
        et_belongs_new_who.setText(data.owner)
    }
