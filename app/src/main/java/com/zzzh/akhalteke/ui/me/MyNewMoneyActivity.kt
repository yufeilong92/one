...
import kotlinx.android.synthetic.main.activity_my_new_money.*
 ...
 * @Purpose :钱包管理
 */
class MyNewMoneyActivity : BaseActivity(), MyMoneyContract.View, CheckInfomContract.View  {
    companion object {
    ...
    override fun setContentView(savedInstanceState: Bundle?) {
        setContentView(R.layout.activity_my_new_money)
        bindViewData()
        initEvent()
        initRefresh()
        clearList()
    ...
    fun bindViewData() {
        tv_my_money_remaining_new_sum.text = Utils.addMoneyComma(mMoney)
    }
    ...
    fun initRefresh() {
        mymoney_gm_new_SmartRefreshLayout.setEnableLoadMore(true)
        mymoney_gm_new_SmartRefreshLayout.setEnableRefresh(false)
        mymoney_gm_new_SmartRefreshLayout.apply {
    ...
    fun initAdapter() {
        setMangager(rlv_mymoneygm_new_content)
        mAdapter = MyMoneyAdapter(mContext, mArrayList!!)
        rlv_mymoneygm_new_content.adapter = mAdapter
    }

    fun initEvent() {

        tv_my_money_add_new_card.setOnClickListener {
            //银行卡管理
            val bundle = Bundle()
            jumpTo(BankListActivity::class.java, bundle, getString(R.string.bank_manager))
        }

        tv_my_money_tixain_new.setOnClickListener {
            //提现
...
//            jumpTo(TiMoneyActivity::class.java, bundle, getString(R.string.tixian))
    ...
    override fun OneSuccess(t: Any?) {
        mymoney_gm_new_SmartRefreshLayout.finishLoadMore()
        if (t == null) return
        val data: MyMoneyTiXianVo = t as MyMoneyTiXianVo
        tv_my_money_remaining_new_sum.text = data.usableAmount
        mMoney = data.usableAmount
        clearList()
        if (data.list == null || data.list.isEmpty()) {
            mymoney_gm_new_SmartRefreshLayout.setEnableLoadMore(false)
            mAdapter!!.setEmptyView(R.layout.gm_refresh_empty, mymoney_gm_new_SmartRefreshLayout)
