...
abstract class SelectorImageFragment : BaseFragment() {

    override fun setContentView(): Int {
        return setContentViewImg()
    }

    override fun setCreatedContentView(view: View, savedInstanceState: Bundle?) {
        setCreatedContentImgView(view, savedInstanceState)
    }

    abstract fun setContentViewImg(): Int
    abstract fun setCreatedContentImgView(view: View, savedInstanceState: Bundle?)
        ...
        selectImageDialog = object : SelectImageDialog(mContext) {
            override fun onTakePrice() {
                PermissionUtils.showPermission(
                        activity!!, "上传图片需要照相和读写权限，是否同意", arrayOf(
                        Permission.CAMERA,
